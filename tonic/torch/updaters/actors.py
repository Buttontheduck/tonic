    ...
    def __call__(self, observations):
        ...
        def weights_and_temperature_loss(q_values, epsilon, temperature):
            ...
            if self.temperature_dependent:
                ...
                temp = temperature.view(1, -1)          # (1, B)  → broadcasts over K

                # E-step: soft weights with per-state temperature
                tempered_q = q_values.detach() / temp   # (K, B)
                weights = torch.softmax(tempered_q, dim=0).detach()

                # Dual loss for the temperature parameters
                q_log_sum_exp = torch.logsumexp(tempered_q, dim=0)   # (B,)
                log_num_actions = math.log(q_values.shape[0])        # scalar

                per_state_loss = temp.squeeze(0) * (epsilon + q_log_sum_exp - log_num_actions)  # (B,)
                loss = per_state_loss.mean()                         # scalar
