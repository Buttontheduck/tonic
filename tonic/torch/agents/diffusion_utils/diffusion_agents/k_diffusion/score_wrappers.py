...
class GCDenoiser(nn.Module):
    ...
    def __init__(self, inner_model, sigma_data=1.):
        super().__init__()
        self.inner_model = inner_model #hydra.utils.instantiate(inner_model)
    ...
    def loss(self, state, action, noise, sigma, **kwargs):
        """
        ...
        # noised_input = action + noise * append_dims(sigma, action.ndim)
        model_output = self.inner_model(state, noised_input * c_in, sigma, **kwargs)
    ...
    def forward(self, state, action, sigma, **kwargs):
        """
        ...
        """
        c_skip, c_out, c_in = [append_dims(x, action.ndim) for x in self.get_scalings(sigma)]
        return self.inner_model(state, action * c_in, sigma, **kwargs) * c_out + action * c_skip
